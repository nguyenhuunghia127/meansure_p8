...
let MetricsService = class MetricsService {
    ...
    async getDashboardOverview() {
        const [projects, modules, requirements, testCases, testResults, testRuns, bugs, codeCoverage, aiAnalysis,] = await Promise.all([
        ...
        const latestResults = this.getLatestByKey(testResults, (result) => result.test_case_id);
        const latestCoverage = this.getLatestByKey(codeCoverage, (coverage) => coverage.module_id);
        const latestAiAnalysis = this.getLatestByKey(aiAnalysis, (analysis) => analysis.module_id);
        const executedCount = latestResults.length;
        const passedCount = latestResults.filter((result) => this.normalizeStatus(result.status) === 'passed').length;
        const failedCount = latestResults.filter((result) => this.normalizeStatus(result.status) === 'failed').length;
        const totalTestCases = testCases.length;
        const totalBugs = bugs.length;
        const productionBugs = bugs.filter((bug) => this.normalizeSource(bug.source) === 'production').length;
        const testingBugs = totalBugs - productionBugs;
        const openBugs = bugs.filter((bug) => this.normalizeStatus(bug.status) === 'open').length;
        const linkedRequirementCount = requirements.filter((requirement) => requirement.requirement_test_cases.length > 0).length;
        const moduleIdsWithTests = new Set(testCases.map((testCase) => testCase.module_id));
        const coverageValues = latestCoverage.map((coverage) => Number(coverage.coverage_percent));
        const avgExecutionTimeSeconds = latestResults.length
            ? this.round(latestResults.reduce((sum, result) => sum + (result.execution_time_seconds ?? 0), 0) / latestResults.length)
            : 0;
        const summary = {
        ...
        const coverage = {
            requirementCoverage: this.toPercent(linkedRequirementCount, requirements.length),
            codeCoverage: coverageValues.length
                ? this.round(coverageValues.reduce((sum, value) => sum + value, 0) /
                    coverageValues.length)
                : 0,
            testCaseCoverage: this.toPercent(moduleIdsWithTests.size, modules.length),
        };
        const requirementsByModule = this.groupByCount(requirements, (requirement) => requirement.module_id ?? -1);
        const linkedRequirementsByModule = this.groupByCount(requirements.filter((requirement) => requirement.requirement_test_cases.length > 0), (requirement) => requirement.module_id ?? -1);
        const testCasesByModule = this.groupByCount(testCases, (testCase) => testCase.module_id);
        const latestResultsByModule = this.groupBy(latestResults, (result) => result.test_cases.module_id);
        const openBugsByModule = this.groupByCount(bugs.filter((bug) => this.normalizeStatus(bug.status) === 'open'), (bug) => bug.module_id);
        const productionBugsByModule = this.groupByCount(bugs.filter((bug) => this.normalizeSource(bug.source) === 'production'), (bug) => bug.module_id);
        const modulesOverview = modules
            .map((module) => {
            const moduleResults = latestResultsByModule.get(module.id) ?? [];
            const modulePassedCount = moduleResults.filter((result) => this.normalizeStatus(result.status) === 'passed').length;
            const moduleExecutionRate = this.toPercent(moduleResults.length, testCasesByModule.get(module.id) ?? 0);
            const modulePassRate = this.toPercent(modulePassedCount, moduleResults.length);
            const moduleRequirementCoverage = this.toPercent(linkedRequirementsByModule.get(module.id) ?? 0, requirementsByModule.get(module.id) ?? 0);
            const moduleCodeCoverage = latestCoverage.find((coverageItem) => coverageItem.module_id === module.id);
            const moduleAi = latestAiAnalysis.find((analysis) => analysis.module_id === module.id);
            ...
            const aiRecommendations = this.extractAiSuggestions(moduleAi?.suggested_test_cases);
            return {
                id: module.id,
                name: module.name,
                ...
                riskScore: moduleAi?.risk_score
                    ...
                    : heuristic.riskScore,
                insufficientTesting: moduleAi?.insufficient_testing ?? heuristic.insufficientTesting,
        ...
        const recentRuns = [...testRuns].reverse().map((run) => {
            const passed = run.test_results.filter((result) => this.normalizeStatus(result.status) === 'passed').length;
            const failed = run.test_results.filter((result) => this.normalizeStatus(result.status) === 'failed').length;
            return {
        ...
        const aiRecommendations = this.uniqueStrings(highRiskModules.flatMap((module) => module.recommendations.map((recommendation) => `${module.name}: ${recommendation}`))).slice(0, 5);
        ...
        return {
            ...
            inputSources: [
                {
                    ...
                    description: 'Repository of requirements and test cases linked to modules.',
                ...
                },
                {
                    ...
                    description: 'Historical run results used for execution and pass/fail metrics.',
                ...
                },
                {
                    ...
                    description: 'Imported defects from Jira, GitHub issues, or manual QA logging.',
                ...
                },
                {
                    ...
                    description: 'Coverage metrics and code-quality signals collected per module.',
            ...
            aiInsights: {
                overview: highRiskModules.length > 0
    ...
    async filterByModule(moduleName) {
        ...
        const modules = moduleName
            ? overview.modules.filter((module) => module.name.toLowerCase() === moduleName.toLowerCase())
            : overview.modules;
        await this.systemService.recordHistory('dashboard', `Applied module filter: ${moduleName ?? 'all'}`);
        return {
    ...
    buildModuleHeuristic(input) {
        ...
        const insufficientTesting = input.testCaseCount === 0 ||
            input.executionRate < 80 ||
            input.requirementCoverage < 80 ||
            input.codeCoverage < 70;
        const recommendations = [];
        if (input.testCaseCount === 0) {
            recommendations.push('Create baseline functional test cases for this module.');
        }
        if (input.requirementCoverage < 80) {
            recommendations.push('Link uncovered requirements to dedicated test cases.');
        }
        if (input.passRate < 80) {
            recommendations.push('Add regression coverage for the scenarios that still fail.');
        }
        if (input.codeCoverage < 70) {
            recommendations.push('Increase unit and integration coverage around risky paths.');
        }
        if (input.productionBugs > 0) {
            recommendations.push('Convert production defects into regression tests before the next release.');
        }
        if (recommendations.length === 0) {
            recommendations.push('Maintain current test depth and monitor change-heavy areas.');
        }
        return {
...
exports.MetricsService = MetricsService = __decorate([
    ...
    __metadata("design:paramtypes", [typeof (_a = typeof prisma_service_1.PrismaService !== "undefined" && prisma_service_1.PrismaService) === "function" ? _a : Object, typeof (_b = typeof system_service_1.SystemService !== "undefined" && system_service_1.SystemService) === "function" ? _b : Object])
], MetricsService);
