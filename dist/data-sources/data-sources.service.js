...
let DataSourcesService = class DataSourcesService {
    ...
    async importFromStructuredFile(file, meta) {
        ...
        if (extension !== 'csv') {
            return {
                ...
                message: 'This MVP import flow currently supports CSV files. XLSX can be added next with a spreadsheet parser package.',
            };
        }
        ...
        await this.systemService.recordHistory('file-import', `Imported ${rows.length} CSV rows for ${importType} from ${file.originalname}`);
        return {
            imported: true,
            fileName: file.originalname,
            importType,
    ...
    async importTestCasesFromRows(rows, meta) {
        const projectName = meta.projectName?.trim() || 'Imported Project';
        const grouped = new Map();
        for (const row of rows) {
            const moduleName = this.getString(row.moduleName) ??
            ...
            if (requirementCode && requirementTitle) {
                ...
                const linkedTestCaseCodes = new Set(this.getStringArray(existingRequirement.testCaseCodes));
                if (testCaseCode) {
        ...
        for (const item of grouped.values()) {
            imports.push(await this.importTestCaseRepository({
                projectName,
    ...
    async importBugTrackingFromRows(rows, meta) {
        const payload = {
            projectName: meta.projectName?.trim() || 'Imported Project',
    ...
    async importCoverageFromRows(rows, meta) {
        const payload = {
            projectName: meta.projectName?.trim() || 'Imported Project',
